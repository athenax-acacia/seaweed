...
#     sage post-manoeuvre-moments.sage --output-folder <some/temp/dir> -v --order 0,1,0,1
# to recreate, then copy the file 'moment_0101.py' on top of this one.
...
def moment_0101(delta_v, mu, sigma):
    ...
    return -delta_v0 * (
        ...
        cos(mu0 - sigma_30)) + delta_v1 * (
            ...
            cos(mu0 - sigma_30) +
            (mu3 - sigma_33) * exp(-mu3 - sigma_00 / 2 + sigma_33 / 2) *
            sin(mu0 - sigma_30)) + mu1 * mu3 + sigma_31
...
\frac{1}{2} \, \sigma_{33}\right)} \sin\left(\mu_{0} - \sigma_{30}\right)\right)} {\Delta v_0} + {\left(\sigma_{30}
...
\, \sigma_{33}\right)} \sin\left(\mu_{0} - \sigma_{30}\right)\right)} {\Delta v_1} + \mu_{1} \mu_{3} + \sigma_{31}
"""
