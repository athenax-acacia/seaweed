...
#     sage post-manoeuvre-moments.sage --output-folder <some/temp/dir> -v --order 0,1,0,2
# to recreate, then copy the file 'moment_0102.py' on top of this one.
...
def moment_0102(delta_v, mu, sigma):
    ...
    return -delta_v0 * (
        ...
        (-sigma_30 * exp(-mu3 - sigma_00 / 2 + sigma_33 / 2) *
         ...
         cos(mu0 - sigma_30)) * (mu3 + sigma_30 + sigma_33)) + delta_v1 * (
             sigma_33 * exp(-mu3 - sigma_00 / 2 + sigma_33 / 2) *
             sin(mu0 - sigma_30) +
             (sigma_30 * exp(-mu3 - sigma_00 / 2 + sigma_33 / 2) *
              cos(mu0 - sigma_30) +
         ...
         ) + mu1 * sigma_33 + mu3 * sigma_31 + mu3 * (mu1 * mu3 + sigma_31)
...
{\left(\mu_{3} + \sigma_{30} + \sigma_{33}\right)}\right)} {\Delta v_0} + {\left(\sigma_{33} e^{\left(-\mu_{3} -
...
\sigma_{33}\right)}\right)} {\Delta v_1} + {\left(\mu_{1} \mu_{3} + \sigma_{31}\right)} \mu_{3} + \mu_{3} \sigma_{31} +
\mu_{1} \sigma_{33}
"""
