...
#     sage post-manoeuvre-moments.sage --output-folder <some/temp/dir> -v --order 0,1,1,1
# to recreate, then copy the file 'moment_0111.py' on top of this one.
...
def moment_0111(delta_v, mu, sigma):
    ...
    return delta_v0**2 * (
        ...
        cos(2 * mu0 - 4 * sigma_30)) - delta_v0 * (
            ...
            (mu3 - sigma_30 + sigma_33)) - delta_v0 * (
                ...
                ) + mu1 * sigma_32 + mu2 * sigma_31 + mu3 * (mu1 * mu2 +
                                                             sigma_21)


def latex_0111():
    return r"""
\frac{1}{2} \, {\left(2 \, \sigma_{30} \cos\left(2 \, \mu_{0} - 4 \, \sigma_{30}\right) e^{\left(-2 \, \mu_{3} - 2 \,
...
\sigma_{33}\right)} \sin\left(2 \, \mu_{0} - 4 \, \sigma_{30}\right)\right)} {\Delta v_1}^{2} - {\left(\sigma_{32}
...
{\left(\mu_{3} + \sigma_{30} + \sigma_{33}\right)}\right)} {\Delta v_0} - {\left(\sigma_{31} e^{\left(-\mu_{3} -
...
\sigma_{33}\right)}\right)} {\Delta v_0} - {\left(\sigma_{31} \cos\left(\mu_{0} - \sigma_{30}\right) e^{\left(-\mu_{3} -
...
\sigma_{30} + \sigma_{33}\right)}\right)} {\Delta v_1} + {\left(\mu_{1} \mu_{2} + \sigma_{21}\right)} \mu_{3} + \mu_{2}
\sigma_{31} + \mu_{1} \sigma_{32}
"""
