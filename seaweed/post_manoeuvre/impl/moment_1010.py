...
#     sage post-manoeuvre-moments.sage --output-folder <some/temp/dir> -v --order 1,0,1,0
# to recreate, then copy the file 'moment_1010.py' on top of this one.
...
def moment_1010(delta_v, mu, sigma):
    ...
    return -delta_v0 * (
        ...
        sin(mu0 - sigma_30)) - delta_v1 * (
            ...
            sin(mu0 - sigma_30) +
            (mu0 - sigma_30) * exp(-mu3 - sigma_00 / 2 + sigma_33 / 2) *
            cos(mu0 - sigma_30)) + mu0 * mu2 + sigma_20
...
\frac{1}{2} \, \sigma_{33}\right)} \sin\left(\mu_{0} - \sigma_{30}\right)\right)} {\Delta v_0} - {\left({\left(\mu_{0} -
...
\sin\left(\mu_{0} - \sigma_{30}\right)\right)} {\Delta v_1} + \mu_{0} \mu_{2} + \sigma_{20}
"""
