...
#     sage post-manoeuvre-moments.sage --output-folder <some/temp/dir> -v --order 1,0,1,1
# to recreate, then copy the file 'moment_1011.py' on top of this one.
...
def moment_1011(delta_v, mu, sigma):
...
def latex_1011():
    return r"""
-{\left(\sigma_{30} e^{\left(-\mu_{3} - \frac{1}{2} \, \sigma_{00} + \frac{1}{2} \, \sigma_{33}\right)}
...
{\left(\mu_{3} - \sigma_{30} + \sigma_{33}\right)}\right)} {\Delta v_0} - {\left(\sigma_{30} \cos\left(\mu_{0} -
...
\sigma_{33}\right)}\right)} {\Delta v_1} + {\left(\mu_{0} \mu_{2} + \sigma_{20}\right)} \mu_{3} + \mu_{2} \sigma_{30} +
\mu_{0} \sigma_{32}
"""
