...
#     sage post-manoeuvre-moments.sage --output-folder <some/temp/dir> -v --order 1,0,1,2
# to recreate, then copy the file 'moment_1012.py' on top of this one.
...
def moment_1012(delta_v, mu, sigma):
    ...
    return -delta_v0 * (
        ...
        (mu3 - sigma_30 + sigma_33)) - delta_v1 * (
            ...
            (mu3 + sigma_30 + sigma_33)) + mu3 * (
                mu0 * sigma_32 + mu2 * sigma_30 + mu3 * (mu0 * mu2 + sigma_20)
            ) + sigma_30 * (mu2 * mu3 + sigma_32) + sigma_32 * (
                mu0 * mu3 + sigma_30) + sigma_33 * (mu0 * mu2 + sigma_20)


def latex_1012():
    return r"""
-{\left({\left(\sigma_{30} e^{\left(-\mu_{3} - \frac{1}{2} \, \sigma_{00} + \frac{1}{2} \, \sigma_{33}\right)}
...
\, \sigma_{33}\right)} \sin\left(\mu_{0} - \sigma_{30}\right)\right)} \sigma_{33}\right)} {\Delta v_0} -
...
\sin\left(\mu_{0} - \sigma_{30}\right)\right)} \sigma_{33}\right)} {\Delta v_1} + {\left({\left(\mu_{0} \mu_{2} +
