...
#     sage post-manoeuvre-moments.sage --output-folder <some/temp/dir> -v --order 1,1,0,2
# to recreate, then copy the file 'moment_1102.py' on top of this one.
...
def moment_1102(delta_v, mu, sigma):
    ...
    return -delta_v0 * (
        ...
        (mu3 + sigma_30 + sigma_33)) + delta_v1 * (
            ...
            (mu3 - sigma_30 + sigma_33)) + mu3 * (
                mu0 * sigma_31 + mu1 * sigma_30 + mu3 * (mu0 * mu1 + sigma_10)
            ) + sigma_30 * (mu1 * mu3 + sigma_31) + sigma_31 * (
                mu0 * mu3 + sigma_30) + sigma_33 * (mu0 * mu1 + sigma_10)


def latex_1102():
    return r"""
-{\left({\left(\sigma_{30} \cos\left(\mu_{0} - \sigma_{30}\right) e^{\left(-\mu_{3} - \frac{1}{2} \, \sigma_{00} +
...
\sin\left(\mu_{0} - \sigma_{30}\right)\right)} \sigma_{33}\right)} {\Delta v_0} + {\left({\left(\sigma_{30}
...
\sin\left(\mu_{0} - \sigma_{30}\right)\right)} \sigma_{33}\right)} {\Delta v_1} + {\left({\left(\mu_{0} \mu_{1} +
\sigma_{10}\right)} \mu_{3} + \mu_{1} \sigma_{30} + \mu_{0} \sigma_{31}\right)} \mu_{3} + {\left(\mu_{1} \mu_{3} +
\sigma_{31}\right)} \sigma_{30} + {\left(\mu_{0} \mu_{3} + \sigma_{30}\right)} \sigma_{31} + {\left(\mu_{0} \mu_{1} +
