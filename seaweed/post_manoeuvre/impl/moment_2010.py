...
#     sage post-manoeuvre-moments.sage --output-folder <some/temp/dir> -v --order 2,0,1,0
# to recreate, then copy the file 'moment_2010.py' on top of this one.
...
def moment_2010(delta_v, mu, sigma):
    ...
    return -delta_v0 * (
        ...
        (sigma_00 * exp(-mu3 - sigma_00 / 2 + sigma_33 / 2) *
         ...
         sin(mu0 - sigma_30)) * (mu0 - sigma_00 + sigma_30)) - delta_v1 * (
             sigma_00 * exp(-mu3 - sigma_00 / 2 + sigma_33 / 2) *
             cos(mu0 - sigma_30) +
             (-sigma_00 * exp(-mu3 - sigma_00 / 2 + sigma_33 / 2) *
              sin(mu0 - sigma_30) + (mu0 - sigma_30) *
              exp(-mu3 - sigma_00 / 2 + sigma_33 / 2) * cos(mu0 - sigma_30)) *
             (mu0 + sigma_00 + sigma_30)) + 2 * mu0 * sigma_20 + mu2 * (
                 mu0**2 + sigma_00)


def latex_2010():
    return r"""
-{\left(\sigma_{00} e^{\left(-\mu_{3} - \frac{1}{2} \, \sigma_{00} + \frac{1}{2} \, \sigma_{33}\right)}
...
{\left(\mu_{0} - \sigma_{00} + \sigma_{30}\right)}\right)} {\Delta v_0} - {\left(\sigma_{00} \cos\left(\mu_{0} -
...
\sigma_{30}\right)}\right)} {\Delta v_1} + {\left(\mu_{0}^{2} + \sigma_{00}\right)} \mu_{2} + 2 \, \mu_{0} \sigma_{20}
"""
