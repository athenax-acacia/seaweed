...
#     sage post-manoeuvre-moments.sage --output-folder <some/temp/dir> -v --order 2,1,0,1
# to recreate, then copy the file 'moment_2101.py' on top of this one.
...
def moment_2101(delta_v, mu, sigma):
        ...
        (mu3 + sigma_30 + sigma_33)) + delta_v1 * (
            ...
            (mu3 - sigma_30 + sigma_33)) + mu3 * (
                ...
                (mu0**2 + sigma_00)) + 2 * sigma_30 * (
                    mu0 * mu1 + sigma_10) + sigma_31 * (mu0**2 + sigma_00)
...
{\left({\left(\mu_{0}^{2} + \sigma_{00}\right)} \mu_{1} + 2 \, \mu_{0} \sigma_{10}\right)} \mu_{3} + 2 \, {\left(\mu_{0}
\mu_{1} + \sigma_{10}\right)} \sigma_{30} + {\left(\mu_{0}^{2} + \sigma_{00}\right)} \sigma_{31}
"""
