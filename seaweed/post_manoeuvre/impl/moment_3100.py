...
#     sage post-manoeuvre-moments.sage --output-folder <some/temp/dir> -v --order 3,1,0,0
# to recreate, then copy the file 'moment_3100.py' on top of this one.
...
def moment_3100(delta_v, mu, sigma):
    ...
    return -delta_v0 * (
        ...
        (mu0 + sigma_00 + sigma_30)) + delta_v1 * (
            ...
            (mu0 - sigma_00 + sigma_30)) + mu1 * (
                2 * mu0 * sigma_00 + mu0 *
                (mu0**2 + sigma_00)) + 3 * sigma_10 * (mu0**2 + sigma_00)
...
{\left({\left(\mu_{0}^{2} + \sigma_{00}\right)} \mu_{0} + 2 \, \mu_{0} \sigma_{00}\right)} \mu_{1} + 3 \,
{\left(\mu_{0}^{2} + \sigma_{00}\right)} \sigma_{10}
"""
